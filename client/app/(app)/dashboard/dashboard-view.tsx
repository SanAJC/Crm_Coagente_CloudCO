...
import { CalendarDays, CircleDollarSign, Receipt, UtensilsCrossed, Users } from "lucide-react";

import { AppShell } from "@/components/app-shell";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { ReservationsCalendar } from "@/components/reservations-calendar";
import { currency, stageLabels, ticketTotal, todayISO } from "@/lib/crm-data";
import { useCrm } from "@/lib/crm-store";

export function DashboardView() {
  const { reservations, tickets, products } = useCrm();

  const todays = reservations.filter((r) => r.date === todayISO && r.status !== "cancelada");
  const guests = todays.reduce((sum, r) => sum + r.people, 0);
  const openTickets = tickets.filter((t) => t.stage !== "cerrado");
  const sales = tickets.reduce((sum, t) => sum + ticketTotal(t), 0);
  ...
  const kpis = [
    ...
    },
    {
      ...
      hint: `${tickets.filter((t) => t.stage === "cocina").length} en cocina`,
      icon: Receipt,
    },
    {
    ...
    <AppShell
      ...
      actions={
        <>
          ...
          <Button asChild>
            <Link href="/pedidos">Crear ticket</Link>
          </Button>
        </>
      }
    >
      ...
      <div className="mt-6 grid gap-4 lg:h-[640px] lg:grid-cols-3">
        ...
        <section className="panel flex min-h-0 flex-col p-5">
          <h2 className="text-lg font-medium">Tickets en curso</h2>
          <ul className="mt-4 min-h-0 flex-1 space-y-4 overflow-auto pr-1">
                  ...
                  <div className="space-y-2 p-3">
                    ...
                    <div className="flex items-center justify-between gap-2 rounded-md bg-accent px-2 py-1.5 text-accent-foreground">
                      ...
                      <span className="text-[11px] font-medium">
                        {ticket.items.reduce((sum, item) => sum + item.qty, 0)} ítems
                      </span>
                      <Badge variant="outline">{stageLabels[ticket.stage]}</Badge>
                    </div>
                    <p className="truncate text-xs text-muted-foreground">{ticket.customer}</p>
                  </div>
            ...
            {openTickets.length === 0 ? (
              <li className="text-sm text-muted-foreground">Sin tickets abiertos.</li>
            ) : null}
          </ul>
        </section>
      </div>
